...
src_install() {
	...
	insinto "${dir}"
	doins -r *
	fperms 755 "${dir}"/bin/{clion,clion.sh,fsnotifier,inspect.sh,ltedit.sh,repair,restarter,clang/linux/x64/{clangd,clang-tidy,clazy-standalone,llvm-symbolizer}}
	...
	dosym "${EPREFIX}${dir}/bin/${PN}" "/usr/bin/${PN}"
